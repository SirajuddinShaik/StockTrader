...
class DataIngestion:
    ...
    def download_new_data(self, interval = 30):
        ...
        for i in range(len(symbols)):
            uri = uri_format.format(api = os.environ["TWELVE_DATA_API"], interval = interval, start_date = ll, end_date = ul, symbol = symbols[i].strip())
            ...
            df.to_csv(self.config.local_data_file + f"{symbols[i].strip()}.csv")
            logger.info(f">>>>>>>>>> {symbols[i].strip()}.csv saved to artifacts <<<<<<<<")
