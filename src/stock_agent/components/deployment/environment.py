...
from stock_agent.pipeline.stage_05_model_evaluation import (
...
class DeploymentStockMarketEnv:
    ...
    def take_action(self, action):
        ...
        if self.cash > self.min_balance:
            ...
            new_stocks = np.array(
                new_stocks
                / np.maximum(self.data[-1][26:].reshape((20, 5))[:, self.close], 1e-8),
                dtype=np.int16,
            )
            for i in range(self.num_stocks):
                stock_price = self.data[-1][26:].reshape(20, 5)[i][self.close]
    ...
    def update_curr_prices(self):
        ...
        concat = np.concatenate(
            (self.stocks, [self.cash], [self.portfolio_value], new_data)
        )
    ...
    def update_portfolio(self, new_data):
        self.previous_portfolio_value = self.portfolio_value
        self.portfolio_value = self.cash + np.sum(
            self.stocks * new_data[4:].reshape((self.num_stocks, 5))[:, self.close]
        )
    ...
    def cash_withdraw(self, cash, status):
        ...
        if status and self.cash - self.min_balance >= cash:
            ...
            self.portfolio_value = self.cash + np.sum(
                self.stocks * np.array(self.manager.prices)
            )
            message = "sucessful"
        self.manager.transaction("withdraw", "INR", cash, cash, 0, message)
    ...
    def cash_deposit(self, cash, status):
        ...
        if status:
            ...
            message = "sucessful"
        self.manager.transaction("deposit", "INR", cash, cash, 0, message)
    ...
    def buy_stocks(self, id, quantity, status):
        ...
        self.manager.transaction(
            "USER-buy", self.symbols[id], quantity, cost, 0, message
        )
    ...
    def sell_stocks(self, id, quantity, status):
        ...
        self.manager.transaction(
            "USER-sell", self.symbols[id], quantity, cost, 0, message
        )

    def render(self, mode="human", close=False):
        profit = self.portfolio_value - self.initial_cash
        print(
            f"Step: {self.current_step}, Cash: {self.cash}, Stocks: {self.stocks}, Portfolio Value: {self.portfolio_value}, Profit: {profit}"
        )
